      ...
      function renderParams(params, current) {
        ...
        for (const [key, meta] of Object.entries(params)) {
          ...
          if (meta.type === 'enum' && meta.values) {
            ...
            html += '<div style="display:flex; align-items:center; gap:8px;"><input type="checkbox" '+(value===true||value==='true'?'checked':'')+' onchange="up(\\''+key+'\\',this.checked,\\'bool\\')"> <span style="font-size:12px;">Active</span></div>';
          } else if (meta.type === 'int' && meta.range) {
            html += '<div class="range-container"><input type="range" class="param-input" min="'+meta.range[0]+'" max="'+meta.range[1]+'" value="'+value+'" oninput="this.nextElementSibling.innerText=this.value" onchange="up(\\''+key+'\\',this.value,\\'int\\')"><span class="range-value">'+value+'</span></div>';
